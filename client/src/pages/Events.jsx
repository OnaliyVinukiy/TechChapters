...
function Events() {
  ...
  useEffect(() => {
    ...
    onValue(eventsRef, (snapshot) => {
      ...
      for (let id in eventData) {
        ...
        uniqueClubNames.add(eventData[id].clubName);
      }

      setEvents(eventList);
      setClubNames(Array.from(uniqueClubNames));
      setLoading(false);
    });
  }, []);
  ...
  const filteredEvents = events.filter((event) => {
    ...
    return (
      (selectedClub ? event.clubName === selectedClub : true) &&
      (date ? eventDate.toDateString() === date.toDateString() : true) // Compare dates only if a date is selected
    );
  });
